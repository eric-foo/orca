...
| search_discovery | Exact product and brand queries | available_now | Find hidden venues, better origin surfaces, and negatives. | SERP routes, query language, absent/present product-specific surfaces. | to_retrieve | not_applicable | not_applicable | Search is route discovery, not proof. |
...
| SBR-007 | search_discovery | exact public queries | Product and discontinued-term searches | Hidden venues, better origin surfaces, decisive absence | search_interest | source_route | counterevidence_path | medium | to_retrieve | Query, retrieval date, result route, no-yield note | not_applicable | not_applicable | Route-discovery row for scanning; not a proof row. |
...
classifier_handoff_packet:
  ...
  source_family_gaps:
    ...
    - SBR-007 search-discovery rows should look for hidden venues and decisive negatives.
