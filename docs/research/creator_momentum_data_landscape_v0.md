...
|---|---|---|---|---|---|---|
| **Global YouTube Trending 2022–25** (Illinois Data Bank) | trending-*presence* proxy (⚠️ not a per-item panel) | YouTube | **No** — daily trending-*list* snapshots (726K unique vids, mass turnover; can't follow one video over time) | views + comments (no likes/shares) | **CC BY 4.0 — commercial-clear** | free (26 GB) |
| **ksb2043** (Kim, WWW'20) | IG validation | Instagram | post-level timestamps (pseudo-trajectory; not a panel) | likes+comments+caption+hashtags; 9 categories incl. beauty; 34K creators/10.2M posts | form-gated; license **unstated** (research-purpose implied, not binding) | free |
| **TikTok-10M** (HF, The-data-company) | quality classifier | TikTok | No (snapshot; create+collected_time, Spring 2025) | richest free: play/digg(=like)/comment/share/collect(=save); **~6.65M rows (name overstates "10M")** | "other" + TikTok ToS → **research-only** | free |
| **tarekmasryo YT/TikTok Trends 2025** | ⚠️ **SYNTHETIC** — schema/EDA template only, NOT a real-data benchmark | YT + TikTok | No (generated, not measured) | save/share/comment/like-rate columns exist but **values are synthetic** (hex IDs, placeholder handles) | CC BY 4.0 (data not real) | free |
...
> **Foundation verification (2026-06-13, live-fetch):** two original-pass claims were corrected in the table above — **Global-YT-Trending is trending-list *snapshots*, not a same-videos panel** (so its "backtest spine" billing is weaker than stated), and **tarekmasryo is synthetic data**, not a real-measured benchmark. ksb2043 (has post timestamps; license unstated) and TikTok-10M (~6.65M rows, not 10M) had minor corrections. These rows are now fetch-verified, superseding the original "as-reported by sources" caveat.

**The 3-step proof plan:**
1. **Discriminability backtest** — Global-YT-Trending (trending-*presence* pattern — **NOT a per-item trajectory**, see correction above) + ksb2043 (IG; label outcomes by looking creators up *now*) → does early signal sort durable from transient, beating the "hottest-now" baseline. *(Caveat, now fetch-confirmed: Global-YT-Trending is daily trending-list **presence** — different videos churn through the list, so you cannot follow one video's 3-yr arc; it proves the **method** on presence, not organic trajectory. **For a real per-item trajectory spine, the verified YouNiverse (weekly subs+views, →2019) or TubeCensus (subscriber trajectory, post-2022) are stronger — swapping the spine is an owner decision.**)*
2. **Quality classifier** — develop "what high-quality engagement looks like" on the snapshot set **TikTok-10M** (**tarekmasryo dropped — synthetic data**).
