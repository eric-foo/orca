...
- anchor_goal: Define the **at-scale IG capture operating envelope** for the beauty vertical — proxy cost/benefit, a human-like-behavior model beyond the runner's `bounded_jitter`, and the **ideal operating-account count** — grounded on the measured **per-IP, pace-bound R**; and finish the **at-pace volume ceiling**.
...
- **decision:** how to operate IG capture at full-vertical scale, given R is per-IP and pace-bound.
  - options / sub-questions (owner-supplied, all coupled):
    1. **Proxy cost for scrolling/at-scale** — proxies were ruled out for *realistic cadences* (pace discipline suffices), but cost the proxy/IP-rotation option anyway as a **perma-block fallback** and for if scroll-depth / volume grows. Cost vs marginal throughput.
    2. **Human-like behavior** — beyond `bounded_jitter` (8–45s gaps): scroll patterns, variable session length, time-of-day shaping, idle gaps. Lower takedown risk + stay "faster-than-human-not-takedown-risking."
  ...
  - trade-offs: proxy $ vs near-zero current benefit (pace-bound, not volume-bound); human-likeness effort vs takedown-risk reduction; account count bounded by the ≤10 cap AND by per-IP economics.
