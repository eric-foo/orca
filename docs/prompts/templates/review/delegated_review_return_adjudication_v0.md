...
3. Decide the material cleanliness state. If any blocker, major, material unresolved issue, or material uncertainty remains, stop downstream planning and set `next_action` to the smallest complete closure route for that issue.
4. Only if no unresolved material issue remains, collapse all admin/lifecycle work into exactly one land step. Then deep-think the next 1-3 material moves that need judgment. Admin does not count as a material move.
...
adjudication_closeout:
  ...
  residuals: []
  admin_land_step: "<exactly one step, only when status is clean; otherwise omit or null>"
  next_material_steps:
    - step: "<material step, not admin>"
      why_it_compounds: "<short reason>"
      main_risk: "<short risk>"
  next_action: "<closure route when material issue remains; otherwise land step first, then material moves if any>"
  non_claims:
    - not validation
    - not readiness
