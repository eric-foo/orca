# Core Spine v0 — Signal Statement Record Architecture Direction v0

```yaml
retrieval_header_version: 1
artifact_role: Product architecture direction brief
scope: Owner-decided DIRECTION for the wedge-agnostic Signal Statement Record — the structured "what a signal says" layer atop the existing provenance (SourceCapturePacket) and integrity (ECR postures) layers. Locks direction + invariants + shape, not the field-by-field schema.
use_when:
  - Scoping or reviewing the signal-statement layer of the Data/Cleaning/ECR spine.
  - Checking what the bounded statement-field ratification does and does not authorize.
...
`OWNER_DECIDED_DIRECTION` — recorded 2026-06-09. Bounded statement-field ratification GRANTED.

Name-propagation note (2026-06-18): active routes, docs, package paths, model
names, deriver API, tests, and deterministic row ids now use the full **Signal
Statement Record** name. This is a naming and routing clarification only: not
validation, not readiness, not backend/storage selection, not Evidence Unit
field-architecture completion, and not a JSG-01 claim change.
...
The wedge-agnostic **Signal Statement Record** — the structured *"what a signal says"* layer. Today content exists only as prose (the IPF Evidence Unit Standard fields, collapsed at the harness into the single `EvidenceUnit.summary: str` — verified `orca-harness/schemas/case_models.py:64`) and as a human rubric (Engagement Logic Registry, not a machine row schema). The gap is a structured, family-typed, judgment-decoupled statement row that can be aggregated, compared, and fed to a decision.

## Owner decisions (settled 2026-06-09)

1. **Carrier — option (b), durable.** A parallel derived `SignalStatementRecord`, keyed to the `SourceCapturePacket`, composed by the Evidence Unit **by reference** — the **second derived-record kind** after the ECR integrity postures. It is the next instance of the pattern the system already runs: "a parallel record the frozen conductor reads, binds no `EvidenceUnit`" (verified `orca-harness/ecr/__init__.py:26,37-38`). It folds into option (a) [content as a field on the Evidence Unit] cheaply if the owner later re-conceives the Evidence Unit as the content object; option (c) [the IPF Evidence Unit Standard] is the **vocabulary this record structures**, not a competing carrier.
...
4. **Authority — bounded statement-field ratification GRANTED.** The content analogue of the SP-1/2/3/6 source-side *integrity* exception. It does **not** settle the final Evidence Unit field architecture (reserved — verified boundary doc:131-133, 253-254) and does **not** unfreeze JSG-01 (the gate stays FROZEN; the source-side fields were ratified, the gate was not — boundary doc:289, ecr `__init__`.py:26).
...
- **Common spine (family-invariant):** `statement_id` · `signal_family` (closed enum: the four statement-row families + `RESIDUAL_FAMILY_UNRESOLVED`; signal-quality meta is not a statement row — see Family 5 below) · `subject_entity` (who/what the signal is *about* — not the source) · `event_or_claim` (the observed substance, raw, pre-interpretation) · `signal_event_time` (by reference to `PacketTiming`, distinct from capture time and cutoff) · `decision_relevance` (the neutral tag, #3) · `raw_observation` (verbatim, source-language anchor).
...
direction_change_propagation:
  doctrine_changed: >
    ...
    a bounded statement-field ratification that does NOT settle the reserved final
