...
- **Acceptance:** (a) a fixture where Wayback has no pre-cutoff memento but archive.today does → the ladder locates + retrieves it; `archives_tried` records the Wayback miss then the archive.today hit; `archive_selected = archive_today`; (b) an all-miss fixture → honest NO-GO after the bounded ladder, each miss recorded; (c) the per-archive `<=cutoff` guard excludes a post-cutoff memento on any archive; (d) the single-archive Wayback-hit path is byte-identical to today (no regression); (e) `timetravel.mementoweb.org` is not referenced anywhere.
- **Owner-deferrable (from the slice-E pre-build assumption gate):** whether archive.today is in the v1 ladder or deferred to a later slice, given its reachability cost (the ladder degrades gracefully to Wayback + publisher-history without it).
...
- **Proposed shape:** `retrieve_body(url, *, pinned_rung=None, max_rung, cutoff_context)`. No `pinned_rung` → **probe**: escalate direct-HTTP → anti-blocking-HTTP → headless browser → **cloakbrowser+proxy** (the same `cloakbrowser_snapshot.py` machinery the Reddit screening-read route already uses against Reddit anti-bot), stop at the first usable body, return `body_rung_used`. With `pinned_rung` → **steady-state**: fetch direct at that rung; on failure, re-probe (escalate from the pinned rung). The caller **records the working rung per archive / source-family** — the capture-recon-index already records cheapest-GO-rung per source, so the pin reuses that pattern (no new state store; build turn decides doc-vs-runtime record). The slice-E orchestrator calls this per located memento.
...
- **Proposed shape:** `verify_archive_body(result, *, requested_url, requested_timestamp, cutoff)` returns the verdict the slice-F dispatcher consumes as its "usable body" gate. The cutoff check is **per-archive**: Memento archives (Wayback, archive.today) carry the *served* date in `Memento-Datetime` (**observed** — see below), so assert `served_ts <= cutoff`; the publisher-history rung fetches a *requested revision id*, so revision-identity is the cutoff guarantee there (no served-date header). Also assert URL-identity match and content-integrity markers (archive not-found placeholder detection; decode/charset sanity). **On a post-cutoff served date, do not just NO-GO — signal slice E to re-select the next-older pre-cutoff snapshot** (E↔G loop); reach NO-GO only when no pre-cutoff snapshot yields a clean body. Reuses the existing limitation-note machinery.
