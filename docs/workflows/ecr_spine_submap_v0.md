...
scope: Single repo submap that orients a reader across the ECR source-side derived-record spine — the integrity postures (ECR SP-1/2/3/6) and the sibling statement layer (Signal Statement Record), the deriver discipline they share, and the frozen boundary to the JSG-01 conductor. Map only; not source-of-truth.
use_when:
  - Orienting to the ECR source-side spine (integrity postures or Signal Statement Record) before reading or extending any derived-record code or plan.
  - Finding which owner doc owns an ECR posture, the Signal Statement Record layer, the shared deriver discipline, or a deferred surface.
  - Checking what is built vs declared-but-dormant vs frozen, without bulk-loading every ECR/Signal Statement Record artifact.
...
stale_if:
  - A new derived-record kind is added beside the ECR integrity postures and the Signal Statement Record.
  - The shared M1/M2/M3 carry-or-residualize discipline or the per-kind grain invariant changes in an owner doc.
  - The authored-interpretation lane (signal_family + event-core) is built (activates the dormant Signal Statement Record M2 seam).
...
> source to open for an ECR source-side / Signal Statement Record question, and states
...
- **Statement** — *what the signal says.* The Signal Statement Record (`orca-harness/signal_statement/`).

The cross-kind invariants the spine runs on (stated here for orientation only — the owner doc holds the authority):

1. **Reference, never merge.** Statement records reference provenance (`packet_id`/`slice_id`) and integrity (ECR posture keys) **by key**, one-directional statement → provenance. Linked, never collapsed: statement is "what's said," integrity is "can I trust the saying." → *Signal Statement Record direction brief.*
2. **One derived record per epistemic kind, composed by the Evidence Unit, never merged.** The integrity postures and the statement row are siblings at their own grain; future kinds (e.g. corroboration) follow the same shape rather than sprawling. → *Signal Statement Record direction brief.*
3. **Carry-supplied-or-residualize; never author from prose.** Every source-side deriver is a pure function of its frozen inputs (the packet plus caller-supplied bodies / posture-keys / authored-interpretation). Each field is carried from an input or emitted as a **named residual**; an absent input → honest residual. No deriver classifies, selects, or infers an interpretive value from packet prose (the shared M1-carry / M2-derive / M3-residual discipline). → *Signal Statement Record deriver plan (Amendment v0.1); ECR frame plan.*
...
| I need to... | Open |
| --- | --- |
| Understand the statement layer's locked direction + invariants | `docs/product/signal_statement/core_spine_v0_signal_statement_record_architecture_v0.md` |
| Build/review the Signal Statement Record deriver (carry-or-residualize, per-slice grain, the D2 event-time amendment) | `docs/product/signal_statement/signal_statement_record_deriver_architecture_plan_v0.md` (see **Amendment v0.1**) |
...
| Inspect the built Signal Statement Record deriver + model | `orca-harness/signal_statement/` |
...
| Inspect the tests | `orca-harness/tests/unit/test_ecr_*`, `orca-harness/tests/unit/test_signal_statement_*`, `orca-harness/tests/unit/test_finalization_models.py`, `orca-harness/tests/unit/test_run_finalization_receipt.py` |
...
| Check logical data-lake mechanics across capture, projection, ECR/Signal Statement Record, Cleaning, and Judgment | `docs/product/core_spine/core_spine_v0_data_lake_mechanics_map_v0.md` |

## Current Reality Snapshot

- **Built + committed (clean working tree; unit tests under `orca-harness/tests/unit/`):** the four ECR integrity postures — SP-1 identity, SP-2 inspectability, SP-3 timing-cutoff, SP-6 source-visibility — in `orca-harness/ecr/`, and the Signal Statement Record deriver in `orca-harness/signal_statement/`. Each at its own true grain; pure; binds no Evidence Unit. *Implementation reality is the code — check it there; plan docs may run ahead of or behind it. Review history lives in the owning plan docs and the commit log.*
- **Declared-but-dormant:** the Signal Statement Record **authored-interpretation lane** (`signal_family` + event-core). The deriver residualizes that core today (the default) because no authored-classification input / SP-5-style finalizer exists in source. A named, typed seam — not built (the SP-6 precedent).
...
- **Cross-layer mechanics map:** `docs/product/core_spine/core_spine_v0_data_lake_mechanics_map_v0.md`
  ...
  ECR/Signal Statement Record, Cleaning, and Judgment. It does not select storage, schema, or
  implementation.
...
- **Statement:** direction + invariants — `docs/product/signal_statement/core_spine_v0_signal_statement_record_architecture_v0.md`; deriver contract — `docs/product/signal_statement/signal_statement_record_deriver_architecture_plan_v0.md`. Code: `orca-harness/signal_statement/`.
