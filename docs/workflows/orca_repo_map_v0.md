...
| `orca-harness/runners/run_data_lake_catalog.py` | CLI entrypoint for the generated Bronze Catalog v0: inspect-only by default, nonzero on missing/stale/orphaned generated catalog files, reports source-surface coverage, and replaces `indexes/derived_retrieval/bronze_catalog/v0` from verified raw packets only when `--rebuild` is explicitly set. |
