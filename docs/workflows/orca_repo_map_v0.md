...
- Route addition carried from PR 371: Google search-surface route guard added (`.agents/hooks/check_search_surface_google_route.py` enforces the checkable shell of `docs/decisions/search_surface_google_parameterized_us_capture_route_v0.md` for changed durable docs, with advisory PostToolUse hooks and a diff-scoped CI gate; route-shape only, not physical locality, validation, readiness, demand proof, Judgment evidence, or Product Lead evidence).
