...
| `orca-harness/tests/` | `unit/`, `contract/`, and `integration/` tests plus fixtures, including no-LLM-import, no-tools contract guards, Fragrantica MGT capture wrapper and projection/Cleaning/lake tests, Commission Signal Board validator fixtures, CSB-first scanning artifact checker fixtures, and creator public-handle linkage synthetic/adversarial fixtures. |
