...
| `orca-harness/runners/` | CLI entrypoints for case runs, memorization probe, bounded Candidate URL Intake live first-contact sourcing, Reddit Graph Frontier / crawling graph register preparation, source-capture packets (incl. the cross-archive `run_source_capture_historical_packet.py`, which runs the slice-E locate ladder and writes the selected rung's body plus a thin `archive_locate_metadata.json` ladder receipt), offline projection materialization such as `run_ig_creator_momentum_projection.py`, the locked-list IG calls batch circuit guard `run_source_capture_ig_calls_batch.py` (wraps the per-profile IG calls packet runner, stops/cooldowns on circuit-break signals, no source discovery or comment capture), local Retail/PDP packet-directory projection (`run_retail_pdp_projection.py`; no capture, ECR, Cleaning, or Judgment), the opt-in Retail/PDP CloakBrowser projection sidecar (`run_source_capture_cloakbrowser_packet.py --source-family retail_pdp --retail-pdp-projection-output <path>`), and source-observability reports. Retail/PDP binding/residual implementation lives in `orca-harness/source_capture/retail_pdp_projection.py`. The runners named here are illustrative, not exhaustive (~33 total; enumerate with `git ls-files orca-harness/runners/*.py`). |
