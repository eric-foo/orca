...
| `docs/product/judgment_spine/judgment_level1_todo_harness_reconciliation_v0.md` | Level 1 todo/harness reconciliation map — open after the current-state/decomposition map when choosing the next Level 1 docs slice. Distinguishes active-lane organizers, existing product-learning cases, sibling-only prompt/commission work, and missing forecast/action/log/evaluation/receipt pieces; not prompt adoption, case admission, run authorization, validation, readiness, or proof. |
...
- `docs/product/judgment_spine/judgment_level1_todo_harness_reconciliation_v0.md` when choosing the next Level 1 todo/harness slice or checking whether a prompt, commission, case, forecast, action, log, evaluation, or receipt surface is active-lane, sibling-only, or missing.
