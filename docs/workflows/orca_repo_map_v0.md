...
| `orca-harness/tests/` | `unit/`, `contract/`, and `integration/` tests plus fixtures, including no-LLM-import, no-tools contract guards, Fragrantica projection/Cleaning/lake tests, Commission Signal Board validator fixtures, CSB-first scanning artifact checker fixtures, creator public-handle linkage synthetic/adversarial fixtures, and TikTok SCI single-video and parsed-batch admission parser/runner tests. |
