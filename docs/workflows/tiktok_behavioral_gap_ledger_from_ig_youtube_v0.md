...
| --- | --- | --- | --- | --- |
| Profile/listing grid | First-slice recon observed profile page fields and 16 video permalinks in a real cookied browser: profile identity/bio/link/post items/video-view text plus raw follower integer. | `requires_live_probe` plus `code-enforceable` | Existing evidence is N=1 and logged-out/cookied-browser oriented; sessioned/cookied behavior, challenge rate, listing depth, and selector drift are not measured. | Keep profile/listing as TikTok adapter-owned. Enforce stable item IDs and source-surface provenance; do not borrow IG grid selectors or YouTube channel mechanics. |
