...
def _superseded_receipt_ids(receipts: list[FinalizationReceipt]) -> set[str]:
    return {receipt.supersedes for receipt in receipts if receipt.supersedes is not None}


def select_current_receipt(receipts: list[FinalizationReceipt]) -> FinalizationReceipt | None:
    """The single un-superseded receipt, or None when zero or more-than-one remain.

    Callers should pass receipts already scoped to one evidence_id.
    """
    if not receipts:
        return None
    superseded = _superseded_receipt_ids(receipts)
