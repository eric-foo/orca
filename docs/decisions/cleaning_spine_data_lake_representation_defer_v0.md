...
- **Engagement-context implication:** a future cleaned `engagement_context` record is a per-packet Cleaning-derived Silver record under this same shape, not Gold and not a new storage tier. Cross-packet engagement comparisons or clusters wait for the same assembly / `derived_retrieval` triggers as other cross-packet Cleaning output.
