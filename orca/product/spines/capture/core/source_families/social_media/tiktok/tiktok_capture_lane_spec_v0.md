...
- **New (TikTok satellite):** `tiktok_parse.py` (blob + comment-list parsers), a non-headless capture runner (C1-C3, C6: warmed context, request cap, comment-panel post-load action, comment/list response predicate), `tiktok_projection.py`. The required change vs the failed recon runs is **running the existing browser observation seam non-headless/sessioned with assets allowed**, not forged request logic.
