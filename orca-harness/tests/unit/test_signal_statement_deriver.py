"""Tests for the v0 Signal Statement Record deriver (carry-supplied-or-residualize).
...
residual default; key-integrity + deterministic statement_id over keys only;
...
from signal_statement.deriver import _BODY_BOUNDARY, _statement_id, derive_signal_statement
...
def test_residualize_honestly_with_no_authored_input() -> None:
    packet = _packet()
    records = derive_signal_statement(
...
def test_event_time_residual_is_the_v0_default() -> None:
    ...
    rec = derive_signal_statement(_packet(), preserved_bodies={"f1": "x"})[0]
...
def test_derived_record_round_trips_under_extra_forbid() -> None:
    rec = derive_signal_statement(_packet(), preserved_bodies={"f1": "body"})[0]
...
def test_one_record_per_body_bearing_slice_in_order() -> None:
    ...
    records = derive_signal_statement(packet, preserved_bodies={"f1": "one", "f2": "two"})
...
def test_slice_with_no_files_emits_no_record() -> None:
    ...
    records = derive_signal_statement(packet, preserved_bodies={"f1": "body"})
...
def test_empty_supplied_body_emits_no_record() -> None:
    records = derive_signal_statement(_packet(), preserved_bodies={"f1": "   "})
    assert records == []
...
# --- key integrity + statement_id ---


def test_statement_id_is_deterministic_over_keys_only() -> None:
    packet = _packet([{"id": "s1", "files": [("f1", "a" * 64)]}], packet_id="pkt-9")
    rec_a = derive_signal_statement(packet, preserved_bodies={"f1": "body A"})[0]
    rec_b = derive_signal_statement(packet, preserved_bodies={"f1": "DIFFERENT body B"})[0]
    # same keys -> same statement_id regardless of body (re-derive stability)
    assert rec_a.statement_id == rec_b.statement_id
    assert rec_a.statement_id.startswith("signal-statement::sha256::")


def test_statement_id_is_tuple_safe_for_separator_bearing_keys() -> None:
    # distinct key tuples must not collide even when an id contains "::"
    assert _statement_id("a::b", "c") != _statement_id("a", "b::c")


def test_ecr_posture_refs_default_empty_and_deduped() -> None:
    rec_default = derive_signal_statement(_packet(), preserved_bodies={"f1": "b"})[0]
    assert rec_default.references.ecr_posture_ref_ids == []
    rec_refs = derive_signal_statement(
...
def test_re_derive_determinism_and_purity() -> None:
    ...
    first = derive_signal_statement(packet, preserved_bodies=bodies)
    second = derive_signal_statement(packet, preserved_bodies=bodies)
    assert first == second
...
def test_multi_body_slice_with_one_missing_body_emits_no_record() -> None:
    ...
    packet = _packet([{"id": "s1", "files": [("f_a", "a" * 64), ("f_b", "a" * 64)]}])
    assert derive_signal_statement(packet, preserved_bodies={"f_a": "PART A"}) == []
...
def test_residual_reasons_carry_state1_not_state2_marker() -> None:
    rec = derive_signal_statement(_packet(), preserved_bodies={"f1": "b"})[0]
    for reason in (
