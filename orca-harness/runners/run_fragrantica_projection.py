...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Project one existing Fragrantica Source Capture Packet into "
            "view-only current-window fragrance-review rows."
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        parser.exit(status=2, message=f"fragrantica projection failed: {exc}\n")
